...
int main(int argc, char *argv[]){
	...
	if (wirefile.is_open()){
		while (!wirefile.eof()){
			...
			if (line.find("Wiring Method : Automatic")){automatic=true;}
			if (line.find("Fixture Size : Bank 1")){jobsize=1;}
			if (line.find("Fixture Size : Bank 2")){jobsize=2;}
			if (line.find("Fixture Size : Full")){jobsize=3;}
			if (line.find("*+*+* Top *+*+*")){top=true;}
	...
	for (long i=0; i<extras.size(); i++){
		comfile << "X" <<	extras[i].x << "," <<
					...
					percentencode(extras[i].name) << endl;
	}
